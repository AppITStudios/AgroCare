...
EasyMarkerCluster.prototype = {
    ...
    initMarker: function(){
        ...
        google.maps.event.addListener(this.marker, 'click', function(){
            for (var i=0; i<parent.easy_markers.length;i++){
                parent.easy_markers[i].show();
                
                (function(){
                    var mark = parent.easy_markers[i];
        ...
        this.map.map_markers.push(this);
        
        var grados = 360 / this.marker_data.length;
        var grados_actual = grados;
        
        for (var i=0; i<this.marker_data.length; i++){
            var radians = grados_actual * (Math.PI / 180);
            var y = 0.004*Math.sin(radians);
            var x = 0.004*Math.cos(radians);
            
            var mark = new EasyMarker({
                latitude: this.marker_data[i].lat + y,
                longitude: this.marker_data[i].lng + x,
            }, this.map);
            this.easy_markers.push(mark);
            mark.setInfoContent(this.marker_data[i].desc);
            mark.setInfoWindow(this.map.infoWindow);
            mark.hide();
